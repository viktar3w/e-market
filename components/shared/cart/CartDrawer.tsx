...
const CartDrawer = ({ className, children }: CartDrawerProps) => {
  ...
  return (
    <Sheet>
      ...
      <SheetContent className={cn('flex flex-col justify-between pb-0 bg-[#f4f1ee]', className)} aria-describedby="">
        <SheetHeader>
          <SheetTitle className="font-bold">{cart.qty} item(s)</SheetTitle>
        </SheetHeader>
        <div className="-mx-6 mt-5 overflow-auto flex-1 scrollbar">
          {cart.cartItems.map((item) => (
            <CartDrawerItem key={item.id} item={item} loading={cart.loading} className="mb-2" />
          ))}
        </div>
        <SheetFooter className="-mx-6 bg-white p-8">
          <div className="w-full">
            <div className="flex mb-4">
              ...
              <span className="font-bold text-lg">{formatPrice(cart.totalAmount)}</span>
            </div>
            <Link href="/checkout/">
