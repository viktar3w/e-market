...
import { useGetProductByNameLazyQuery } from '@/documents/generates/hooks/apollo';
...
const HeaderSearch = ({ className }: HeaderSearchProps) => {
  ...
  const [searchProducts, { data, loading, error }] = useGetProductByNameLazyQuery();
  useEffect(() => {
    if (!!error) {
  ...
  useEffect(() => {
    if (!!data?.products?.items) {
      setProducts(data.products.items as Product[]);
    }
  }, [data]);
  useClickAway(ref, () => {
    setFocused(false);
  ...
  return (
    <>
      ...
      <div
        ref={ref}
        className={cn('flex rounded-lg md:rounded-2xl flex-1 justify-between relative h-10 md:h-11 z-30', className)}
      >
